...
**Reflection**: is this a better representation? Do you have a better
idea?

**Preview**: intrinsic uncertainty in the data (sans the artificial
jitter!)
...
+----------------+-----------------------------------------+
| Uncertainty    | From data/model outputs                 |
| type           |                                         |
+================+=========================================+
| (Intrinsic)    | ?                                       |
+----------------+-----------------------------------------+
...
2.  Know how to show (the different types of) uncertainty.
    **PREDICTION**: which representation is most *expressive*?
...
uncertainty? What are they for? Which is wider and why?
...
Notes on calculating the `se.pred`:
...
what you want. Remember the scatterplot we made earlier?
...
**REFLECTION**: How good is this model comparison?

**REFLECTION**: Any questions plotting points?
...
To specify a student-t distribution, we need three parameters:
...
We won't calculate CI confidence *intervals* directly. Instead we'll use
the standard errors and use `distributional::dist_student_t` and
`ggdist` functions.

Note: `dist_student_t` only works with `xdist` aesthetic from the
`ggdist` package.

The spec for a dot-and-whisker plot:
...
stat_intveral x \<- .epred x \<- .prediction x \<- effectiveness y \<-
condtion
...
**PREDICTION**: how should it look like? Write a spec below
...
```{r}
# EXERCISE: adapt the frequentist version for `means_t_bayes`
means_t_bayes %>%
  sample_draws(30) %>%
  ggplot(
    aes(y = condition)) + 
...
**REFLECTION**: why does the Kruschke spaghetti density show both PI and
CI? How else can you show PI for the Bayesian model? Hints:

-   Many prediction distribution implied by the posterior
-   Single posterior predictive distribution integrated over parameter
    uncertainty

**REFLECTION**: should you show confidence or prediction distribution,
or both? Should you use intervals or distributions for uncertainty?
...\_usability\_
...
EXERCISE: On the probability scale. One design: ![](images/000002.png)
...
EXERCISE: compare the two conditions ![](images/000006.png)
