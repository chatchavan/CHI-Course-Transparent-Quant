...
Last time, we modeled how `condition` (graph or no graph) affected `effectiveness`,
...
t-test is equivalent to a linear model estimating two means. For the frequentist version, we are using `lm` because the output is easier to wrangle (good explainer:
...
**Prediction interval (PI), or prediction uncertainty**
...
EXERCISE: plot estimates on the cumulative log odds scale
...
EXERCISE: On the probability scale
