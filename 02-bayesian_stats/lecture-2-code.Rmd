...
```{r}
model.4 = brm(pumps ~ 1 + condition + trial + (trial | participant),
...
# draws.model.4 = 
wrap_plots(
  plot_predictions(model.4, df = crossing(condition = c('expansive', 'constrictive'), 
                                          trial = 0:29,
                                          participant = 0)), 
  p1,
nrow = 2)
...
We generate the average of 30 trials.
...
wrap_plots(
  model.4.posteriors %>% 
 ggplot(aes(x = .epred, fill = condition)) +
    geom_histogram(alpha = .5, size = 1, binwidth = 5, color = NA) +
    scale_color_theme() +
...
model.4.posteriors %>% 
ggplot() +
  geom_histogram(aes(x = .epred, fill = condition), alpha = .5, color = NA) +
...
model.4.posteriors_diff %>% 
ggplot() +
  geom_histogram(aes(x = .epred), alpha = .5,  fill = theme_blue, color = NA, adjust = 2) +
