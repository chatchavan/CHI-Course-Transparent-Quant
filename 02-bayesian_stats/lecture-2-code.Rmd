...
log(\mu) &= \beta_{0} + \beta_{0,j} + \beta_{1} * x_i \\
\beta_{0} &\sim \mathrm{Normal}(3.5, 0.5) \\
\beta_{0, j} &\sim \mathrm{Student\_t}(3, 0, 1) \\
\beta_{1} &\sim \mathrm{Normal}(0, 0.5) \\
\phi &\sim \mathrm{Gamma}(3, 1) \\
i & \in \{\mathrm{expansive}, \mathrm{constrictive}\}\\
j & \in \{1, ..., \mathrm{N}\}
\end{align}
$$
...
model.4.prior_pred = brm(pumps ~ 1 + condition + (1 | participant),
                      ...
                      prior = c(prior(normal(3.5, 0.5), class = Intercept),
                                prior(normal(0, 0.5), class = b),
                                prior(student_t(3, 0, 1), class = sd),
                                prior(gamma(3, 1), class = shape)
                      ),
                      backend = BRM_BACKEND,
                      sample_prior = "only",
