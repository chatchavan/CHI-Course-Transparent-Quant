...
log(\mu) &= \beta_{0} + \beta_{1} . x_i \\
\beta_{0} &\sim \mathrm{Normal}(4.1, 0.5) \\
\beta_{0, j} &\sim \mathrm{Student\_t}(3, 0, 1) \\
\beta_{1} &\sim \mathrm{Normal}(0, 0.5) \\
\phi &\sim \mathrm{Gamma}(5, 1) \\
i & \in \{\mathrm{expansive}, \mathrm{constrictive}\}\\
\end{align}
$$
...
\begin{align}
y_{i} &\sim \mathrm{Neg-Binomial}(\mu, \phi) \\
log(\mu) &= \beta_{0,k} + \beta_{1} . x_i + \beta_j . x_j  \\
\beta_{0, j} &\sim \mathrm{Normal}(\alpha, \sigma) \\
\alpha &\sim \mathrm{Normal}(4.1, 0.5) \\
