...
p1 = pose_df %>%
  ...
  scale_x_continuous(limits = c(-40, 202), breaks = seq(-40, 202, by = 20))
...
```{r}
head(model.1.priorsamples)
```

```{r  fig.height=2.5}
model.1.priorsamples %>% 
  ggplot(aes(x = .epred, group = condition)) +
  geom_density(alpha = .5, color = NA, fill = theme_yellow) +
  theme_density_x + 
  ggtitle('Prior checks of model.1')
```
...
plot_predictions <- function(model, title = ''){
  model %>% 
    ...
    scale_x_continuous(breaks = seq(-200, 250, by = 100)) +
...
```{r}
plot_posteriors <- function(model, title = ''){
  model %>% 
    epred_draws(tibble(condition = c('expansive', 'constrictive')), 
...
```{r}
cowplot::plot_grid(plot_predictions(model.2), p1, nrow = 2)
```


```{r}
cowplot::plot_grid(plot_posteriors(model.2), p1, nrow = 2)
```
...
model.2.posteriors %>% 
  ...
  ggplot() + 
  geom_density(aes(x = .epred, fill = condition), color = NA, alpha = .5) +
  facet_grid(model ~ .) +
  scale_x_continuous(limits = c(-40, 100), breaks = seq(-40, 100, by = 20)) +
  scale_color_theme() +
  theme_density_x +
  ggtitle('Comparing model.1 with model.2')
```
...
$$
\begin{align}
y_{i} &\sim \mathrm{Neg-Binomial}(\mu, \phi) \\
...
```{r}
model.3.formula = brm(pumps ~ 1 + condition + trial + (trial | participant),
...
```{r}
# draws.model.3 = 
data_grid(pose_raw_df, condition, trial) %>%
  add_epred_draws(model.3.formula, re_formula = NA) %>%
  ggplot() +
  stat_halfeye(aes(x = .epred, y = condition, fill = condition), alpha = .5)
```
