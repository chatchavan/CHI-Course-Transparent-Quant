...
```{r dataset1-olr-comparison-2, fig.height=5, fig.width=3}
dataset1.olr.posterior_comparison  %>%
       ungroup() %>%
       mean_qi(.epred) %>%
  ggplot() +
...
```{r}
dataset2.yprior <-  posterior_predict(dataset2.brm.student.priorchecks, ndraws = 1000)

ggplot() +
  geom_histogram(aes(x = dataset2.yprior),
               fill = DARK_PURPLE,
               alpha = .5, size = 1, center = 0) +
 # scale_x_continuous(breaks = 1:9, limits = c(0.5, 9.5)) + 
 labs(x = 'Prior predictive distribution',  y = "") +
 theme(
   axis.text.y = element_blank()
