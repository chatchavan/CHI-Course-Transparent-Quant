...
posterior_predictions <- tibble(condition = c('graph', 'no_graph')) %>%
  add_predicted_draws(dataset1.brm.bayesiant, re_formula = NA, allow_new_levels = T)
...
kable(as_tibble(get_prior(dataset2.brm.student.formula, data = dataset2)))
...
posterior_samples <- tibble(condition = c(TRUE, FALSE)) %>%
  add_fitted_draws(dataset2.brm.student, re_formula = NA, allow_new_levels = T) 


posterior_predictions <- tibble(condition = c(TRUE, FALSE)) %>%
  add_predicted_draws(dataset2.brm.student, re_formula = NA, allow_new_levels = T) 

kable(head(posterior_samples))

kable(head(posterior_predictions))

```


```{r plot-posterior_intervals, fig.height=2, fig.width=10}
posterior_samples %>%
  mean_qi() %>%
  ggplot() +
  geom_pointrange(aes(
...
```{r plot-posterior_distribution, fig.height=3, fig.width=10}

posterior_predictions %>%
  ggplot(., aes(x = .prediction, fill = condition)) +
  geom_histogram(
    dataset2,
    mapping = aes(x = change, y = (..count..) / sum(..count..)),
    alpha = 0.5,
    fill = 'gray',
    color = NA,
    binwidth = 4
  ...
  facet_grid(~ condition) +
  scale_fill_brewer(palette = "Set1") +
  xlab('posterior change') + ylab('freq') +
