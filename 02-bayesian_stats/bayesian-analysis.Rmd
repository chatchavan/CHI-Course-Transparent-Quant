...
```{r dataset1-bayesiant-pop, fig.height=6, fig.width=6}

newdata <- tibble(condition = c('graph', 'no_graph'))
prior_preds <-  newdata %>%
      add_predicted_draws(dataset1.brm.bayesiant.priorchecks)   %>%
      select(condition, .prediction)  %>% 
      rename(prior_prediction = .prediction) 
    

posterior_preds <-  newdata %>%
      add_predicted_draws(dataset1.brm.bayesiant)   %>%
      select(condition, .prediction)  %>% 
      rename(posterior_prediction = .prediction)


g0 <- exp1.data %>%
ggplot(mapping = aes(x = effectiveness, y = ..density..)) +
  geom_point(
    exp1.data,
    mapping = aes(x = effectiveness, y = 0),
    position = position_jitter(height = .02, width = .2),
    size = .3,
    color = 'darkgray'
  ) +
  coord_cartesian(xlim = c(-1, 12), ylim = c(-0.1, 0.4)) +
...
g1 <- g0 + geom_histogram(
  ...
  geom_density( color = 'darkgray', size = 1, adjust = 3) 
    

g2 <- g1 + geom_density(
    prior_preds,
    mapping = aes(x = prior_prediction),
    color = 'forestgreen',
    fill = NA,#'forestgreen',
    alpha = .3,
    size = 1,
...
g3 <- g2 + geom_density(
    posterior_preds,
    mapping = aes(x = posterior_prediction),
    color = DARK_PURPLE,
    fill = NA,#DARK_PURPLE,
    alpha = .3,
    size = 1,
