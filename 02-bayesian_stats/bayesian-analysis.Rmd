...
get_prior(dataset1.brm.bayesiant.formula, data = exp1.data)
...
bind_rows(
  tidy.wrapper(dataset1.lm.freqt),
...
posterior_samples <- tibble(condition = c(TRUE, FALSE)) %>%
  add_fitted_draws(dataset2.brm.student, re_formula = NA, value = "posterior_change") %>%
...
tidy.wrapper(dataset2.brm.student) %>%
  ggplot() +
  geom_pointrange(
    aes(
      x = term, 
