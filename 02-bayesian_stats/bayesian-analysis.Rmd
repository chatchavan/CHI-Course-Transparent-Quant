...
prior_preds <-  newdata %>%
      add_epred_draws(dataset1.brm.bayesiant.priorchecks)   %>%
      select(condition, .epred)  %>% 
      rename(prior_prediction = .epred) 
    

posterior_preds <-  newdata %>%
      add_epred_draws(dataset1.brm.bayesiant)   %>%
      select(condition, .epred)  %>% 
      rename(posterior_prediction = .epred)

likelihood <- 
  exp1.data %>%
  group_by(condition) %>%
  summarise(mu = mean(effectiveness), sd = sd(effectiveness))
...
ggplot(mapping = aes(x = effectiveness, y = ..density..)) +
  ...
  coord_cartesian(xlim = c(-1, 12), ylim = c(-0.1, 3)) +
