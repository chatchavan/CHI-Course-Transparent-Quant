...
```{r dataset1.bayesiant.diff, fig.height=2, fig.width=5}
dataset1.bayesiant.posterior_epred %>%
...
```{r dataset1-olr-prediction-draws}
dataset1.olr.posterior_prediction <-
  tibble(condition = c('graph', 'no_graph')) %>%
  add_predicted_draws(dataset1.brm.olr2,
                  re_formula = NULL,
                  allow_new_levels = FALSE) %>%
  ungroup()

head(dataset1.olr.posterior_prediction)

dataset1.olr.posterior_prediction  %>% 
  ggplot(.) + 
  geom_bar(aes(fill = .prediction, x = condition), position = 'fill') + 
...
```{r dataset1-olr-comparison-2, fig.height=4, fig.width=3}
dataset1.olr.posterior_comparison  %>%
  ggplot() +
  geom_hline(aes(yintercept = 0), linetype = "dashed", color = "gray") +
  geom_density(
...
```{r dataset1-olr-comparison-3, fig.height=4, fig.width=3}
dataset1.olr.posterior_comparison  %>%
       ungroup() %>%
       mean_qi(.epred) %>%
  ggplot() +
...
```{r}
min(dataset2$adjPDiff)
max(dataset2$adjPDiff)
