...
df1 %>% 
  ...
  geom_point() + 
  geom_label_repel(data = tibble(x =1.8, y = 1.8 * 0.18 + 6.57),
                   mapping = aes(x = x, y = y),
...
```{r}
(condition_no_graph <- tidy(m_olr) %>% slice_tail())
...
broom::augment(m_olr, type.predict = "prob", se_fit = TRUE) %>%
  mutate(effectiveness = as.integer(as.character(effectiveness))) %>%
  group_by(effectiveness, condition) %>%
  slice_head()  %>%
  ggplot(aes(x = effectiveness, y = .fitted, color = condition)) + 
  geom_point() + 
  geom_line(aes(effectiveness, pr_k), data=cumu_df1) + 
  scale_x_continuous(breaks = 1:9)
